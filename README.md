...
Next up is bounding boxes. At this stage it was also observed that Rekognition is more confident detecting "Bird" rather than "Duck" when something that clearly is a rubber duck was uploaded. So another pivot to the approach was made: have Rubber duck confidence be made up of Toy confidence and Bird confidence. (Still a rubber duck if you ask me haha!) The name of the function was also changed and the return value was changed to return a dict. Code to extract the bounding box data was then being written. This is the data that will be used together with matplot lib to draw out the bounding boxes in the images that the use uploads for the final result. Three columns were added to the database schema to store all bounding box data: a Boolean bounding_box_available that is NOT NULL, a Text boolean_box_data, and another Text s3_url_bounding_box.  

To get started with drawing the bounding boxes in matplot lib, ChatGPT provided a function that will draw it using matplotlib; (both pyplot and another one I've never heard of called patches haha.) A lot of changes were done in both app.py and rekognition.py in order to properly extract bounding box data and use it with the uploaded image. The bounding box data needed to be parsed to json format. In rekognition.py, it was decided that the bounding box data for the 'Toy' label will always be used (will often be 98%+) if a rubber duck is detected. The uploaded image also needed to be saved locally in order to use it with PIL and matplotlib and therefore another line of app.config was added to configure UPLOAD_FOLDER.  

(Saving the file locally for drawing bounding boxes caused quite the hick ups. But after brute forcing and a lot of consulting with ChatGPT, I saw the light again hahaha.) The images were saved locally, then upload to S3 and Rekognition fetches the image using from S3. The code to upload to S3 was adjusted slightly from:  
```
try: 
...
### Hyperparameter tuning (all written in 'I' and 'we' form haha)
...
This was an interesting image to upload: https://www.rebeccas.com/rubber-ducks-3-12-inch-12-count.html The 'Toy' label is nowhere to be found, the 'Bird' label is only found with 66.6% confidence and the coin is seemingly interfering a lot. (We can't simply look for 'Bird' and call it a rubber duck, we need 'Toy' or something similar haha.)
